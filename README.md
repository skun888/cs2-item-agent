...
用 Codex、Claude Code、Qoder、Trae 或 WorkBuddy 打开整个 `cs2-item-agent` 文件夹，并按客户端提示信任项目、启用项目级 MCP 或批准本地命令。项目会自动安装依赖、构建程序、迁移本地数据库并创建空白 `.env`，不需要手动执行 npm 命令。
...
完成后直接在 Agent 对话中输入“怎么开始”或提出 CS2 饰品问题。Agent 会自动加载项目 Skill、调用 `health_check` 检查配置，并通过 MCP 选择相应工具。
