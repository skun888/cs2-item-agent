...
当前能力包括库存估值、用户自定义组合告警、可插拔行情适配器、CSQAQ 板块指数与 K 线、本地收藏品/稀有度/汰换关系目录，以及相互独立的行情交易与挂刀执行模型。专属 Skill 已覆盖全部 42 个 MCP 工具，并固化工具路由、证据等级、不确定性表达、副作用确认和跨 Agent 适配规范。

最新公开版本为 [v0.8.0-alpha.1](https://github.com/skun888/cs2-item-agent/releases/tag/v0.8.0-alpha.1)，已作为 GitHub Pre-release 发布：包含 42 个 MCP 工具、12 个 SQLite 迁移和 79 项自动化测试。Windows 与 Linux 均使用 Node.js 24 执行严格类型检查、构建、测试和带强制断言的 MCP 协议验收。当前交付形式为开源代码，尚未提供 npm 包或 Windows 安装程序。

## 快速开始（Agent 用户）

先安装 [Node.js 24 或更高版本](https://nodejs.org/)，然后 Clone 仓库：

```bash
git clone https://github.com/skun888/cs2-item-agent.git
cd cs2-item-agent
```

仓库已为 Codex、Claude Code、Qoder、Trae 和 WorkBuddy 提供项目级兼容配置。用其中一个客户端打开整个 `cs2-item-agent` 文件夹，并按客户端提示信任项目、启用项目级 MCP 或批准本地命令。客户端成功加载配置后，统一启动器会自动安装依赖、构建程序、迁移本地数据库并创建空白 `.env`，不需要手动执行 npm 命令。

打开本地 `.env`，按需填写自己的 `STEAMDT_API_KEY`、`CSQAQ_API_TOKEN`、Steam 代理或企业微信 Webhook，然后重启 MCP。未填写的可选服务不会影响其他已配置能力，`.env` 已被 Git 忽略，不会提交到仓库。

完成后直接在 Agent 对话中输入“怎么开始”或提出 CS2 饰品问题。支持项目 Skill 自动发现的客户端会加载专属 Skill；即使客户端不原生加载 Skill，MCP 也会通过初始化说明和 `health_check` 提供基础使用与配置引导。

Trae Windows 已完成干净 Clone 实机验收；其余四个客户端已通过项目配置自动化契约检查，尚待逐一补充对应客户端的实机验收。若客户端没有自动显示 MCP，请按 [MCP 接入说明](./docs/MCP_SETUP.md) 检查项目级配置入口。
...
以下命令仅供开发、排错和独立验收使用；普通 Agent 用户不需要手动运行。要求 Node.js 24 或更高版本。
...
- 普通缺陷、功能建议和脱敏后的兼容性问题可提交 [GitHub Issue](https://github.com/skun888/cs2-item-agent/issues)；
- 提交代码前请阅读 [贡献指南](./CONTRIBUTING.md)；
- 密钥泄露、任意命令执行、路径穿越或隐私绕过等敏感问题不要公开披露，请按 [安全策略](./SECURITY.md) 报告。
