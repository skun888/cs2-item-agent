...
当前发布基线为 `0.8.0-alpha.1`：包含 42 个 MCP 工具、12 个 SQLite 迁移和 69 项自动化测试。Windows 与 Linux 均使用 Node.js 24 执行严格类型检查、构建、测试和 MCP 协议冒烟；该版本仍是源码预发布基线，不代表已经发布到 npm。
...
阶段 8 的专属 Skill 位于 [`skills/cs2-item-agent`](./skills/cs2-item-agent/SKILL.md)。Skill 负责教 Agent 正确选择工具和解释证据，不生成行情或库存事实；详细市场、挂刀、库存和 DIY 规范按需保存在其 `references/` 目录。非原生支持 Skill 的 MCP 客户端也可以把 `SKILL.md` 与相关参考文件作为项目指令使用。
...
Codex、Qoder 和 Trae 的 MCP 接入方式见 [docs/MCP_SETUP.md](./docs/MCP_SETUP.md)。
